    ...
    <section class="w-11/12 mx-auto mb-4" v-if="hydrated">
        ...
        <div>
            ...
            <div class="grid grid-cols-5 gap-x-4 mb-4 ">
                <UFormField label="Маржа (Націнка)">
                    <UInput v-model="margin" type="number" min="1" placeholder="Напр. 1.4"
                        @change="store.recalculateAll" />
                </UFormField>

                <UFormField label="Курс євро">
                    <UInput v-model="exchangeRate" type="number" placeholder="Напр. 51.1"
                        @change="store.recalculateAll" />
                </UFormField>
                <UFormField label="Знижка">
                    <UInput v-model="discount" type="number" placeholder="Напр. 3" @change="store.recalculateAll" />
                </UFormField>
...
const downloadCSV = () => {
    ...
    const rows = supplyRows.value.map((row) => {
        // Convert the string input to a number and check for decimals
...
const handleSave = async () => {
    ...
    try {
        const payload = {
            ...
            exchange_rate_real: exchangeRate.value,
        ...
        console.log(payload)
        await execute('/supplies/type/eur/', { method: 'POST', body: payload })
        toast.add({
