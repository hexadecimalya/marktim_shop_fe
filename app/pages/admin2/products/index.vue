  ...
  <section class="w-11/12 mx-auto">
    ...
    <div class="flex flex-wrap gap-3 py-4 justify-between items-center">
      <div class="flex gap-4 items-center flex-wrap">
        ...
        <USwitch
          :disabled="!toggleInStock || loading"
          label="Без картинок"
          v-model="toggleNoImages"
          :loading="loading"
        />
      </div>
    ...
    <div v-if="!loading && !error" class="hidden lg:block">
      <div class="grid grid-cols-25 items-center bg-gray-50 border border-gray-200 rounded-t-xl px-2 py-3 text-xs font-semibold uppercase tracking-wider text-gray-500">
        <div>ID</div>
        <div class="col-span-13">Назва</div>
        <div class="col-span-4">Код</div>
        <div class="col-span-5">Категорії</div>
        <div class="flex justify-center"><UIcon name="i-lucide:image" /></div>
      ...
      <div class="divide-y divide-gray-100 border-x border-b border-gray-200 rounded-b-lg">
        ...
        >
          ...
          >
            ...
            <UIcon
              :name="copiedId === product.id ? 'i-lucide:check' : 'i-lucide:clipboard'"
          ...
          <div class="flex justify-end mr-1">
            <UModal
              :key="product.id"
              :open="openedProductId === product.id"
              @update:open="(val) => openedProductId = val ? product.id : null"
              title="Підтвердити видалення"
              :description="`Видалити продукт ${product.name_ukr ?? product.name}?`"
              :ui="{ description: 'font-bold text-black my-4 text-md' }"
            >
              <UButton variant="subtle" color="error" icon="i-lucide:trash" class="p-1.5!" size="xs" @click="openedProductId = product.id" />
      ...
      >
        <div class="flex items-start justify-between gap-2">
          ...
          <UModal
            ...
            :ui="{ description: 'font-bold text-black my-4 text-md' }"
          >
            <UButton variant="subtle" color="error" icon="i-lucide:trash" size="xs" @click="openedProductId = product.id" />
            <template #footer="{ close }">
              <div class="flex justify-end gap-2">
                <UButton label="Скасувати" color="neutral" variant="outline" @click="close" />
                <UButton label="Видалити" color="error" @click="handleDeleteProduct(product.id)" />
