    ...
    <section class="w-11/12 mx-auto mb-4" v-if="hydrated">
        ...
        <div v-else>
            ...
            <div class="grid grid-cols-5 gap-x-4 mb-4">
                ...
                <UFormField label="Курс євро">
                    <UInput v-model="exchangeRate" placeholder="Напр. 51.1" @change="store.recalculateAll" />
                </UFormField>
                <UFormField label="Знижка">
                    <UInput v-model="discount" placeholder="Напр. 3" type="number" @change="store.recalculateAll" />
                </UFormField>
            ...
            <div v-show="counterpartyName && margin && exchangeRate">
                ...
                <!-- Header -->
                <div
                    class="grid grid-cols-11 items-center bg-gray-50 border border-gray-200 rounded-t-xl px-2 py-3 text-xs font-semibold tracking-wider text-gray-500">
                ...
                <div class="border-x border-b border-gray-200 rounded-b-lg">
                    <div v-for="row in supplyRows" :key="row._id"
                        class="grid grid-cols-11 items-center px-2 py-1 text-sm gap-x-1 hover:bg-red-50/20 transition-colors border-b border-gray-100">
                        <!-- Delete -->
                ...
                <div class="flex items-center justify-between mt-3 mb-8">
                    ...
                    <div v-if="supplyRows.length" class="text-sm text-gray-600">
                        <div class="text-end"> Загальна сума:
                            <span class="font-bold text-lg text-gray-900 ml-1 tabular-nums">€ {{ fmt(total) }}</span>
                        </div>
                        <div class="text-end"> Загальна сума (%):
                            <span class="font-bold text-lg text-gray-900 ml-1 tabular-nums">€ {{ fmt(totalPromo)
                                }}</span>
                        </div>
                        <div class="text-end"> Всього:
                            <span class="font-bold text-lg text-gray-900 ml-1 tabular-nums">{{ fmt(totalInUAH) }}
                                грн</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="my-4 flex justify-end bg-gray-100 p-4 rounded-md">
                    <div class="flex items-end justify-end gap-2">
                        <UButton label="Зберегти зміни" color="primary" icon="i-lucide:save" :loading="isSavingSupply"
                            @click="handleUpdate" />
...
const store = useCreateEurSupplyStore();
...
const reloadFromBackend = () => {
    if (supplyData.value) {
        ...
        discount.value = supplyData.value.discount ?? null;
        exchangeRate.value = supplyData.value.exchange_rate_real ?? null;
    }
};
...
const handleUpdate = async () => {
    ...
    try {
        const payload = {
            ...
            type: 'eur',
            total: total.value,
            fidelity_discount: discount.value || 0,
            margin: margin.value,
            total_promo: totalPromo.value || 0,
            total_in_uah: totalInUAH.value,
            exchange_rate_real: Number(exchangeRate.value).toFixed(4) || 0,
            supply_products: supplyRows.value.map((row) => ({
                ...
                sum: row.sum,
                promotion_sum: row.promotion_sum || 0,
            })),
        };

        await execute(`/supplies/${id.value}/type/eur/`, { method: "PUT", body: payload });
        toast.add({
